...
function removeFirstOccurrences(str, value) {
  ...
  return startIndex > -1
    ? str.slice(0, startIndex) +
        str.slice(startIndex + value.length, str.length + 1)
    : str;
}
...
function isPalindrome(str) {
  const findString = str.toLowerCase().replaceAll(/[ ,.*+?!^${}()|]/g, '');
  const palidrome = findString.split('').reverse().join('');
  return palidrome === findString;
}
...
function encodeToRot13(str) {
  const positiveRange = [
    65, 66, 67, 68, 69, 70, 71, 72, 73, 74, 75, 76, 77, 97, 98, 99, 100, 101,
    102, 103, 104, 105, 106, 107, 108, 109,
  ];
  const negativeRange = [
    78, 79, 80, 81, 82, 83, 84, 85, 86, 87, 88, 89, 90, 110, 111, 112, 113, 114,
  ...
  return str
    ...
    .map((item) => {
      ...
      if (positiveRange.includes(char) || negativeRange.includes(char))
        return String.fromCharCode(
          positiveRange.includes(char) ? char + 13 : char - 13
        );
